"""Coordinator for the Packeta parcel tracker integration.
...
from .api import PacketaApiClient, PacketaApiError
from .const import (
...
class PacketaCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for code, result in zip(codes, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (PacketaApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                _LOGGER.warning("Packeta fetch failed for %s: %s", code, result)
        ...
        if codes and errors == len(codes) and not raws:
            raise UpdateFailed("Packeta unreachable for all tracked parcels")
