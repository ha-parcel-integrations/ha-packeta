"""Constants for the Packeta parcel tracker integration."""
...
#
# It is a **POST** whose path carries the tracking number and a ``{locale}``.
# The locale only affects the human event text (status comes from a numeric
# code), so a fixed ``en`` keeps history readable regardless of the user's
# language.
#
# * Success → HTTP 200 ``{"item": {"barcode", "packetStatusId",
