"""Calendar platform for the Packeta parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the Packeta deliveries calendar from a config entry."""
    async_add_entities([PacketaDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class PacketaDeliveriesCalendar(CoordinatorEntity[PacketaCoordinator], CalendarEntity):
    """A read-only calendar of expected Packeta deliveries.
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "Packeta parcel")
            description_parts = [
