"""Sensor platform for the Packeta parcel tracker integration."""
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: PacketaConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Packeta sensor entities from a config entry."""
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                PacketaParcelSensor(self.coordinator, self._entry, barcode)
...
class PacketaAwaitingPickupSensor(
    CoordinatorEntity[PacketaCoordinator], SensorEntity
):
...
class PacketaParcelSensor(CoordinatorEntity[PacketaCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Packeta parcel."""
...
class PacketaNextDeliverySensor(
    CoordinatorEntity[PacketaCoordinator], SensorEntity
):
...
class PacketaDeliveredParcelsSensor(
    CoordinatorEntity[PacketaCoordinator], SensorEntity
):
    """Recently delivered tracked Packeta parcels."""
...
class PacketaLastUpdateSensor(
    ...
    """Diagnostic sensor reporting when Packeta was last polled successfully."""
