"""Packeta parcel tracker custom component for Home Assistant."""
...
from .api import PacketaApiClient
from .const import PLATFORMS
from .coordinator import PacketaCoordinator, _refresh_interval
...
@dataclass
class PacketaData:
    """Runtime data attached to the Packeta config entry."""

    client: PacketaApiClient
    coordinator: PacketaCoordinator


type PacketaConfigEntry = ConfigEntry[PacketaData]


async def async_setup_entry(hass: HomeAssistant, entry: PacketaConfigEntry) -> bool:
    """Set up Packeta from a config entry."""
    # No auth: Packeta tracking is public, so the HA-managed session is fine.
    client = PacketaApiClient(async_get_clientsession(hass))
    ...
    entry.runtime_data = PacketaData(client=client, coordinator=coordinator)
...
async def async_unload_entry(hass: HomeAssistant, entry: PacketaConfigEntry) -> bool:
    """Unload the Packeta config entry."""
