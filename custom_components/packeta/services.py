"""Services for the Packeta parcel tracker integration.
...
def _resolve_entry(hass: HomeAssistant):
    """Return the single Packeta hub, or raise when it is not set up."""
    entries = hass.config_entries.async_entries(DOMAIN)
    if not entries:
        raise ServiceValidationError("Packeta is not set up")
    return entries[0]


def async_setup_services(hass: HomeAssistant) -> None:
    """Register the Packeta services (idempotent)."""
    ...
    async def _track(call: ServiceCall) -> None:
        ...
        if not valid_tracking_code(tracking_code):
            raise ServiceValidationError(
                f"'{tracking_code}' is not a valid Packeta tracking code"
            )
...
def async_unload_services(hass: HomeAssistant) -> None:
    """Remove the Packeta services (single-entry integration, so on unload)."""
