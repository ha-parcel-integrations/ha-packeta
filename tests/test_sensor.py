"""Tests for Packeta sensor property logic."""
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = PacketaParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = PacketaParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_none_without_moments():
    sensor = PacketaNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = PacketaDeliveredParcelsSensor(coordinator, _entry())
