...
def _registered(code: str = ACTIVE_CODE) -> dict:
    """A sample one stage earlier (packetStatusId 997 → registered)."""
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = PacketaApiError("HTTP 500")
...
async def test_fires_delivery_time_changed_event(hass):
    """Suite machinery: a shifted delivery window fires the event.

    Packeta's consumer endpoint exposes no ETA, so ``planned_from`` is always
    ``None`` on a real parcel and this event never fires in practice. The
    branch is shared suite code, so drive ``_fire_change_events`` directly with
    a synthetic window shift rather than pretending Packeta reports one.
    """
    entry = _entry_with([{CONF_TRACKING_CODE: ACTIVE_CODE}])
    entry.add_to_hass(hass)
    coordinator = PacketaCoordinator(hass, AsyncMock(), entry)
    ...
    coordinator._known_state = {ACTIVE_CODE: ParcelStatus.IN_TRANSIT}
    coordinator._known_delivery_times = {ACTIVE_CODE: ("2026-04-29T13:00:00Z", None)}
