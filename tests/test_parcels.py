...
from custom_components.packeta.parcels import (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("31", ParcelStatus.IN_TRANSIT),
        ("2", ParcelStatus.AT_PICKUP_POINT),
        ("3", ParcelStatus.DELIVERED),
        ("21", ParcelStatus.PROBLEM),
    ],
)
...
def test_map_event_status_missing_and_unmapped_are_none():
    ...
    assert map_event_status("3") == ParcelStatus.DELIVERED
...
def test_build_history_caps_to_max_events():
    events = [event("moved", f"2026-04-{day:02d}T10:00:00Z") for day in range(1, 26)]
...
def test_build_history_handles_missing_and_malformed():
    ...
    assert build_history([{"text": "no time"}]) == []  # no time
...
def test_build_history_keeps_unparseable_timestamp_last():
    history = build_history(
        [
            event("fine", "2026-04-24T10:00:00Z"),
            event("odd", "not-a-date"),
        ]
    )
    assert [entry["raw_status"] for entry in history] == ["fine", "odd"]


def test_build_history_text_missing_is_none():
    history = build_history([event("", "2026-04-24T10:00:00Z")])
...
def test_normalize_delivered_parcel():
    ...
    assert parcel["url"] == "https://tracking.packeta.com/en/?id=Z9998887776"
...
def test_normalize_active_parcel_has_no_window():
    ...
    # Packeta consumer tracking carries no delivery-window estimate.
    assert parcel["planned_from"] is None
    assert parcel["planned_to"] is None
...
def test_normalize_pending_placeholder():
    ...
    parcel = normalize_parcel({"trackingNumber": "Z0000000001"})
    assert parcel["barcode"] == "Z0000000001"
